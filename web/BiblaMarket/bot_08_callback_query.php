...
$est=null;
foreach ($DopKnopa as $knop){
	$est=strpos($callbackQuery, $knop.":");
	if ($est!==false){
		$komanda = strstr($callbackQuery, ':', true);	
		$id = substr(strrchr($callbackQuery, ":"), 1);
		$callbackQuery=$komanda;
		$est=null;
	}
}
/*
...
// ОБРАТИ ВНИМАНИЕ  это 3я клавиатура для ввода КОЛИЧЕСТВА которое хотят #продать/#купить	
		...
		num_keybord('1', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key2") { // 2		
		num_keybord('2', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key3") { // 3		
		num_keybord('3', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key4") { // 4		
		num_keybord('4', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key5") { // 5		
		num_keybord('5', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key6") { // 6	
		num_keybord('6', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key7") { // 7		
		num_keybord('7', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key8") { // 8		
		num_keybord('8', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key9") { // 9		
		num_keybord('9', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key10") { // НОЛЬ	
		num_keybord('0', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key11") { // 0 0 		
		num_keybord('20', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key12") { // 0 0 0	
		num_keybord('30', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key13") { // НАЗАД	
		num_keybord('50', 'КОЛИЧЕСТВО монет', $keyInLine3);		
}elseif ($callbackQuery=="key14") { // запятая	
		num_keybord('90', 'КОЛИЧЕСТВО монет', $keyInLine3);		
...
//УСТАРЕВШИЕ КНОПКИ ------------------------------------					
	...
	if ($callback_user_name=='неизвестно') {
		...
		if ($est_li_v_base==false) {
			$query = "INSERT INTO ".$table." VALUES ('". ++$last_id ."', '". $callback_from_id ."' , '" . $callback_first_name . "', 'client', '0')";
			if ($result = $mysqli->query($query)) {	
...
// А ЭТО ОБРАБОТКА КНОПОК PZMarketBot a	
/*
**
**
**
**
*/
}elseif (($callbackQuery==$DopKnopa[0])||($callbackQuery==$DopKnopa[1])||($callbackQuery==$DopKnopa[2])||($callbackQuery==$DopKnopa[3])||($callbackQuery==$DopKnopa[4])||($callbackQuery==$DopKnopa[5])||($callbackQuery==$DopKnopa[6])||($callbackQuery==$DopKnopa[7])||($callbackQuery==$DopKnopa[8])||($callbackQuery==$DopKnopa[9])||($callbackQuery==$DopKnopa[10])||($callbackQuery==$DopKnopa[11])) {
	        
	$query = "SELECT * FROM ".$table5." WHERE otdel='{$callbackQuery}'";
	if ($result = $mysqli->query($query)) {			
		$kol = $result->num_rows;
		if($kol>0){		
			$arrStrok = $result->fetch_all();	
			...
			if ($kol>$max){
				...
				_pechat_lotov($callbackChatId, $arrStrok, $id, --$max);
...
else { // если какая либо кнопка не подошла, может она из предыдущей версии бота, 
		// удаляется старое сообщение и вместо него выводится новое
