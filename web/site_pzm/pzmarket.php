...
while ($a<3){
	...
	if($i>0){
		...
		$id_lota=$arrS[$i][0];	
		$ссылка_на_фото = $ссылка_на_амазон . $id_lota . ".jpg";
		...
		$ссыль_на_телегу = substr(strrchr($caption5, "@"), 1);
		$ссыль_на_телегу = "https://teleg.link/{$ссыль_на_телегу}";		

					
		$текст_лота="<p>{$куплю_продам}</p><p>{$otdel}</p><p><a href={$url}>{$название}</a></p>".
			"<p>{$валюта}</p><p>{$хештеги}</p><p><a href={$ссыль_на_телегу}>{$юзера_имя}</a></p>";			
		
		$массив_лотов[$a]['фото'] = $ссылка_на_фото;
		$массив_лотов[$a]['текст'] = $текст_лота;
	}
