...
if ($arr['message']['reply_markup']){
	if ($arr['message']['reply_markup']['inline_keyboard'][0][0]['text']=='Подробнее'){		
		$urlPodrobno=$arr['message']['reply_markup']['inline_keyboard'][0][0]['url'];
		
		if ($arr['message']['photo']){
			$file_id=$arr['message']['photo'][0]['file_id'];	
			$format="photo";
		}elseif ($arr['message']['video']){
			$file_id=$arr['message']['video']['file_id'];	
			$format="video";
		}
		
		$caption=$arr['message']['caption'];
		if($arr['message']['caption_entities']){
			foreach ($arr['message']['caption_entities'] as $value) {
...
$table2='culc';
// Имя таблицы Mysql в которой хранятся данные о заказе
$table3='zayavka';
// Имя таблицы Mysql в которой хранятся данные о заказе
...
$knopa02 = "";
$knopa03 = "ВЭС";  // \xF0\x9F\x94\x97 
$knopa04 = ""; 
$knopa05 = "Категории товаров";  // \xE2\x9C\x85 
$knopa06 = "";
...
$DopKnopa[0] = "Недвижимость";  // \xF0\x9F\x8F\xA0 
$DopKnopa[1] = "Работа";  // \xF0\x9F\x94\xA8 
$DopKnopa[2] = "Транспорт";  // \xF0\x9F\x9A\x97 
$DopKnopa[3] = "Услуги";  // \xF0\x9F\x92\x87 
$DopKnopa[4] = "Личные вещи";  // \xF0\x9F\x91\x95 
$DopKnopa[5] = "Для дома и дачи";  // \xF0\x9F\x8C\x82 
$DopKnopa[6] = "Бытовая электроника";  // \xF0\x9F\x92\xBB 
$DopKnopa[7] = "Животные";  // \xF0\x9F\x90\xB0 
$DopKnopa[8] = "Хобби и отдых";  // \xE2\x9B\xBA 
$DopKnopa[9] = "Для бизнеса";  // \xF0\x9F\x91\x94 
$DopKnopa[10] = "Продукты питания";
$DopKnopa[11] = "Красота и здоровье";
...
$menu_Admin	= [$stroka[0], $stroka[1], $stroka[2], $stroka[3], $stroka[4]];
//СОЗДАНИЕ КЛАВИАТУРЫ KeybordReply ГЛАВНОГО меню
