		...
		foreach($arrayResult as $stroka){
			...
			if ($stroka['id_admin_group']=='0'&&$stroka['id_chat']!=$chat_id){
				...
				if ($result = $mysqli->query($query)) {
					...
					$tg->sendMessage($from_id, 'Бот добавлен в группу администрирования. Бот должен быть администратором чата, сделайте его администратором!');	
					// А после введите команду /setting, но уже в самой группе АДМИНИСТРИРОВАНИЯ сделок.					
					...
					$tg->sendMessage($admin_group, "БотГарант добавлен в чат-админку ".
						$chat_title);
					
				}else $tg->sendMessage($from_id, 'Не смог добавить группу администрирования в БД');
			}else{
				$query = "UPDATE ".$table6." SET id_chat=". $chat_id .", chat_url='@".$chat_user_name.
