...
include 'Functions_site.php'; // подключение функций работающих с сайтом
...
function _ввод_местонахождения() {
	...
	$reply = "Введите хештеги Вашего местоположения: (не больше трёх)

ВНИМАНИЕ: вводите только страны, области и города. (можно #весь_мир) 

В случае неверного ввода команда PRIZMarket будет вынуждена отклонить заявку, без объяснения причин. 

Каждый хештег должен начинаться с символа # и не должен содержать пробел. Можно использовать буквы, числа и _ (подчёркивание). Никакие другие символы вводить нельзя. 
...
function _отправьте_файл() {
	...
	$reply = "Отправьте один файл. \n\nФото или видео. \n\nЕсли пришлёте больше, я приму только один файл,".
		...
		"(учтите: видео не более 5 МБ и в формате MP4)";	
	...
	if ($результат) {		
		if ($результат->num_rows == 1) {		
			...
			foreach ($результМассив as $строка) {	
				...
				if ($id_client == '7') {
					$количество = substr_count($юзера_имя, '[');
					if ($количество == 0) {
						$связь = _дай_связь_сайт($юзера_имя);
						$юзера_имя = "[{$юзера_имя}]({$связь})";
					} 
				}else $юзера_имя = str_replace('_', '\_', $юзера_имя);
...
function _повтор($номер_лота) {	
	...
	$давно = _последняя_публикация();	
	if ($давно) {
		_отправка_лота($callback_from_id, $номер_лота, true);		
		$inLine = [ 'inline_keyboard' => [ [
	...
	if ($результат) {		
		if ($результат->num_rows == 1) {
			...
			foreach ($результМассив as $строка) {	
				...
				$подробности = "колобок-колобок".$строка['podrobno'];	
				...
				$текст = "\n▪️{$валюта}\n▪️{$хештеги_города}\n▪️";				
				$текст = str_replace('_', '\_', $текст);				
				$текст .= "я точно тебя съем".$юзера_имя."это 100%";
				...
				*/
				...
				$почти_весь_текст = "{$хештеги}{$название_для_подробностей}{$текст}";				

				$imgBB_url = $строка['url_tgraph'];	
				if ($imgBB_url) {								
					$реплика = "[_________]({$imgBB_url})\n{$почти_весь_текст}";					
				}else $реплика = $почти_весь_текст;			
