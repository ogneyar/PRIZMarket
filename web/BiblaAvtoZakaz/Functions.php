...
function _отправить_на_повтор($номер_лота) {
	...
	$bot->sendMessage($admin_group, "Просит {$юзер_неим}: Повторите публикацию, будьте так любезны, заранее благодарю.");
...
function _редактор_лотов($номер_лота) {
	...
	if ($результат) {
		...
		}else throw new Exception("Или нет заказа или больше одного.. (_редактор_лотов)");
	
	}else throw new Exception("Нет такого заказа.. (_редактор_лотов)");	
