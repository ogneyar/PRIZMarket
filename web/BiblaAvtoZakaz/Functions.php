	...
	if ($результат) {
		...
		if ($результат->num_rows == 1) {
			...
			if ($КаналИнфо) {				
				...
				$хештеги_плюс_номер = "{$куплю_или_продам} лот номер {$id_zakaz}\n\n{$категория}\n▪️";		
				$хештеги_плюс_номер = str_replace('_', '\_', $хештеги_плюс_номер);
				
				$текст = "{$хештеги_плюс_номер}[{$название}]({$ссыль_в_названии}){$текст}";
