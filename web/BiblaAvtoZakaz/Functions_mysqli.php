...
**
** _дай_подробности	-	-	-	// функция возврата подробностей из БД
**
	...
	if ($результат) {		
		if ($результат->num_rows == 1) {		
			...
			foreach ($результМассив as $строка) {		
				...
				$подробности = _дай_подробности($номер_лота);
				
				if ($id_client == '7') {
					$связь = _дай_связь_сайт($юзера_имя);
					$имя = "[{$юзера_имя}]({$связь})";
					
					$подпись = "   лот {$номер_лота}   (prizmarket.ru)";
				}else {
					$имя = $юзера_имя; 
					$имя = str_replace('_', '\_', $имя);
					
					$подпись = "   лот {$номер_лота}";
				} 
				...
				if ($inICQ) {
					...
					if ($id_client == '7') {
						...
						if (strpos($юзера_имя, "@")!==false) {
							$имя_icq = "https://teleg.link/".substr($юзера_имя, 1);	
						}else $имя_icq = "https://t.me/".$юзера_имя;
					}
					$текст_после_названия_ICQ = "\n▪️{$валюта}\n▪️{$хештеги_города}\n▪️{$имя_icq}\n{$подпись}";
					if ($доверие) $текст_после_названия_ICQ .= "\n✅ PRIZMarket доверяет❗️";
					$хештеги_ICQ = "{$куплю_или_продам}\n\n{$категория}\n▪️";	
					$текст_ICQ = "{$хештеги_ICQ}{$название}{$текст_после_названия_ICQ}";
					if ($формат_файла == 'фото') {	
						$ссылка_на_файл = $строка['url_tgraph'];
					...
					if ($формат_файла == 'фото') {										
						$vk_file = _дай_файл_вк($номер_лота);						
						if ($vk_file) {						
							$результат = $vk2->wallPost(-$vk_group_id, $текст_вк, $vk_file);						
						}else {
							$результат = $vk2->uploadAndGetUrl($vk_album_id, $vk_group_id, $ссылка_на_файл);
							if ($результат) _запись_ссылок_вк($номер_лота, $результат);
							$результат = $vk2->wallPost(-$vk_group_id, $текст_вк, $результат['vk_file']);			
						}
					}					
...
// функция возврата подробностей из БД
function _дай_подробности($лот) {	
	global $mysqli;		
    $ответ = false;			
	$запрос = "SELECT podrobno FROM `avtozakaz_pzmarket` WHERE id_zakaz='{$лот}'";	
	$результат = $mysqli->query($запрос);	
	if ($результат) {	
		if ($результат->num_rows>0) {			
			$результМассив = $результат->fetch_all(MYSQLI_ASSOC);        
            $ответ = $результМассив[0]['podrobno'];			
		}
	}
    return $ответ;
}
...
// не используется, потому что прописал такую в классе VK 
function _отправка_на_вк($отправляемый_файл) {		
	global $vk2, $vk_album_id, $vk_group_id;	
	$ответ = false;
	
	$результат = $vk2->photosGetUploadServer($vk_album_id, $vk_group_id);	
	//if ($результат['error_msg']) exit;
	$результат = $vk2->upload($результат['upload_url'], $отправляемый_файл);
	...
	if ($ссылка_на_фото) $ответ = $ссылка_на_фото;
...
// функция получения ссылки на файл ВК по номеру лота
function _дай_файл_вк($лот) {		
	global $mysqli;					
	$ответ = false;
	$запрос ="SELECT vk_file FROM `vk_url` WHERE id='{$лот}'";
	$результат = $mysqli->query($запрос);
	if ($результат) {			
		if ($результат->num_rows > 0) {
			$результМассив = $результат->fetch_all(MYSQLI_ASSOC);
			$ответ = $результМассив[0]['vk_file'];
		}
	}	
	return $ответ;	
}
...
// функция записи ссылок ВК
function _запись_ссылок_вк($лот, $массив = []) {		
	global $mysqli;					
	$ответ = false;
	if ($массив) {
		$url = $массив['url'];
		$url_vk = $массив['url_vk'];
		$vk_file = $массив['vk_file'];
		$запрос ="INSERT INTO `vk_url` (
			`id`, `url`, `url_vk`, `vk_file`
		) VALUES (
			'{$лот}', '{$url}', '{$url_vk}', '{$vk_file}'
		)";						
		$результат = $mysqli->query($запрос);								
		if ($результат) {
			$ответ = true;
