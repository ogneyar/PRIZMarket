...
function _отправка_лота($куда, $номер_лота, $админ = false, $предпросмотр = false, $inICQ = false) {	
		...
		if ($результат->num_rows == 1) {		
			...
			foreach ($результМассив as $строка) {		
				...
				if ($предпросмотр) {
					...
					if ($id_client == '7') $подпись = "   лот {$номер_лота}   (prizmarket.ru)";
					else $подпись = "   лот {$номер_лота}";
					$inLine = [	'inline_keyboard' => 
						[ [ [ 'text' => 'Подробнее',
				...
				$хештеги = str_replace('_', '\_', $хештеги);
                                ...
                                //if ($id_client != '7') $текст_после_названия = str_replace('_', '\_', $текст_после_названия);
   ...
   $bot_icq->sendFile($ICQ_channel_market, $фото_на_imgBB, $текст_ICQ, $кнопа);
