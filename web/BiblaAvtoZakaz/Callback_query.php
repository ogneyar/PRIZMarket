...
if ($callback_data=='создать'){
	...
	$reply = "|\n|\n|\n|\n|\n|\n|\n|\nСкиньте мне разом все фото, которые должны оказаться в альбоме (НЕ по одной фотке и НЕ больше 10ти)";	
	$bot->sendMessage($chat_id, $reply, null, $клавиатура_отмена_ввода);	
...
// если клиент нажал кнопку "В начало"
	...
	if (strpos($id, ".")!==false) {				
		$bot->sendMessage($callback_chat_id, "Извините, пока не работает эта возможность.", null, $HideKeyboard);
		//exit('ok');
